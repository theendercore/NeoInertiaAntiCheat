...
public class ServerDataReceiverHandler extends DataReceiverHandler {
    ...
    public ServerDataReceiverHandler(KeyPair keyPair, ResourceLocation modTransferID, ServerLoginPacketListenerImpl handler, DataValidationHandler validator) {
        super(keyPair, modTransferID, handler, validator);
    }

    @Override
    public void receiveMod(MinecraftServer minecraftServer, ServerLoginPacketListenerImpl handler, boolean b, FriendlyByteBuf buf, ServerLoginNetworking.LoginSynchronizer synchronizer, PacketSender packetSender) {
        if (!b) {
